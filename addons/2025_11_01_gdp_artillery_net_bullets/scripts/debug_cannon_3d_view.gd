...
func set_cannon_transform(
	...
	var mid_cannon_position = Vector3(0.0, 0.0, - local_muzzle_height / 2.0)
	# rotate the vector on the horizontal plane
	mid_cannon_position = mid_cannon_position.rotated(Vector3.RIGHT, deg_to_rad(local_elevation_degrees))
	mid_cannon_position = mid_cannon_position.rotated(Vector3.UP, deg_to_rad(world_horizontal_rotation_ccw_360))

	var muzzle_world_position = world_position + mid_cannon_position*2.0 * size_multiplicator
	var cannon_world_position = world_position + mid_cannon_position * size_multiplicator

	if node_anchor_cannon_launcher_to_rotate:
		node_anchor_cannon_launcher_to_rotate.global_position = cannon_world_position
		node_anchor_cannon_launcher_to_rotate.rotation_degrees = Vector3(local_elevation_degrees, world_horizontal_rotation_ccw_360, 0.0)

	if node_muzzle_to_move_3d:
		node_muzzle_to_move_3d.global_position = muzzle_world_position
		node_muzzle_to_move_3d.rotation_degrees = Vector3(local_elevation_degrees, world_horizontal_rotation_ccw_360, 0.0)


	if node_cannon_scaled_tube_middle_3d:
		node_cannon_scaled_tube_middle_3d.global_position = mid_cannon_position * size_multiplicator + world_position*size_multiplicator
		node_cannon_scaled_tube_middle_3d.rotation_degrees = Vector3(local_elevation_degrees, world_horizontal_rotation_ccw_360, 0.0)
		node_cannon_scaled_tube_middle_3d.scale = Vector3(cannon_tube_diameter_meters, cannon_tube_diameter_meters, local_muzzle_height )
	
	if node_root_to_move:
		node_root_to_move.global_position = world_position*size_multiplicator
